...
import { ActivityIndicator, Alert, FlatList, Modal, Platform, Pressable, Text, TextInput, TouchableOpacity, View } from 'react-native';
import ModalAdvert from '../../../components/ModalAdvert';
import ModalError from '../../../components/ModalError';
import { deleteProducto, getProductosPaginated, getReportProducts } from '../../../services/productos';
...
const ListarProductos = ({
  ...
  const handleDownloadPDF = async () => {
    if (datosReporte?.reportePDFBase64) {
      try {
        ...
        const filename = FileSystem.documentDirectory + `Reporte-Inventario-${fecha}.pdf`;
        ...
        if (Platform.OS === "ios") {
          ...
          if (permissions.granted) {
            await FileSystem.StorageAccessFramework.createFileAsync(permissions.directoryUri, `Reporte-Inventario-${fecha}.pdf`, 'application/pdf')
              .then(async (uri) => {
  ...
  const CantidadInput = ({ prod }: { prod: any }) => {
    ...
    const handleBlur = () => {
      let valor = isDecimal ? parseFloat(inputValue) : parseInt(inputValue as any);
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-blue-600 px-4 pt-2 pb-6 shadow-lg">
        {/* Título y selector */}
        <View className="flex-row items-center justify-between mb-4">
          <View className="flex-row items-center gap-3 flex-1">
            ...
            <View className="flex-1">
              <Text className="text-xl font-bold text-white">Productos</Text>
