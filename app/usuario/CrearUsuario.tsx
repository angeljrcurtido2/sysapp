...
import { crearUsuario, crearUsuarioRegistro } from '../../services/usuarios';
import SelectInput from '../clientes/components/SelectInput'; // Re-using the component from clients

interface CrearUsuarioProps {
    onSuccess?: () => void;
    onClose?: () => void;
    isRegistro?: boolean; // Nueva prop para indicar si es desde el registro
}
...
export default function CrearUsuario({ onSuccess, onClose, isRegistro = false }: CrearUsuarioProps) {
    const [successModalOpen, setSuccessModalOpen] = useState(false);
    const [errorModalOpen, setErrorModalOpen] = useState(false);
    const [errorMessage, setErrorMessage] = useState('');
    const [formData, setFormData] = useState({
        ...initialFormState,
        acceso: isRegistro ? 'Administrador' : 'Cajero', // Si es registro, por defecto Administrador
    });
                    ...
                    <View className="mb-4">
                        ...
                        {isRegistro ? (
                            <View className="bg-gray-100 border border-gray-300 rounded-xl px-4 py-3.5">
                                <Text className="text-gray-600">Administrador (fijo)</Text>
                            </View>
                        ) : (
                            <SelectInput
                                name="acceso"
