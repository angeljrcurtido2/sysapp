        ...
        if (permissions.granted) {
          ...
          await FileSystem.StorageAccessFramework.createFileAsync(
            ...
            .then(async (uri) => {
              ...
              Alert.alert('✅ Descarga Completa', `El comprobante de pago "${androidFileName}" se ha guardado en tus descargas.`);
            })
            .catch((e) => {
  ...
  const handlePago = async () => {
    ...
    try {
      ...
      if (response.data?.comprobanteBase64) {
        ...
        Alert.alert(
          "Pago Registrado",
          "El pago se ha registrado correctamente. ¿Desea descargar el comprobante?",
          [
            {
              text: "Cancelar",
              style: "cancel"
            },
            {
              ...
              onPress: async () => {
                await handleDownloadPDF(
                  response.data.comprobanteBase64,
                  `Comprobante-Pago-Deuda-${iddeuda}-${getFormattedDate()}`
                );
              }
            }
          ]
