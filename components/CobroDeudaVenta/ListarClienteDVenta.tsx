...
export default function ListarClienteDVenta({
  ...
  const renderDeudaCard = (item, index) => {
    return (
      <View key={item.iddeuda} className="bg-white rounded-2xl p-4 mb-4 shadow border border-gray-200">
        ...
        <View className="mt-3">
          ...
          <Text className="text-gray-600 text-sm">
            Último pago: {item.ult_fecha_pago ? new Date(item.ult_fecha_pago).toLocaleDateString("es-PY") : "--"}
          </Text>
        </View>
  ...
  return (
    <View className="flex-1 p-4 bg-gray-100">
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        {deudas.length > 0 ? (
          deudas.map((item, index) => renderDeudaCard(item, index))
        ) : (
          <View className="bg-white rounded-2xl p-8 items-center mt-4">
            <Ionicons name="sad-outline" size={64} color="#cbd5e1" />
            <Text className="text-gray-500 mt-4 text-center">No hay deudas para mostrar</Text>
          </View>
        )}
      </ScrollView>
