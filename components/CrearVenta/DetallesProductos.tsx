...
const DetallesProductos: FC<Props> = ({ detalles, setDetalles, tipoDescuento = 'sin_descuento' }) => {
  ...
  const renderDetalleItem = (d: any, idx: number) => {
    ...
    return (
      <View key={idx} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
        ...
        {/* Descuento por producto (si aplica) */}
        {tipoDescuento === 'descuento_producto' && (
          <View className="mt-3">
            <Text className="text-xs text-gray-500 mb-1 font-medium">Descuento (monto)</Text>
            <TextInput
              value={String(d.descuento ?? 0)}
              onChangeText={(value) => updateDetalle(idx, 'descuento', value)}
              keyboardType="numeric"
              className="w-full text-base border-2 border-gray-300 rounded-lg px-3 py-2"
              placeholder="0"
            />
          </View>
        )}
