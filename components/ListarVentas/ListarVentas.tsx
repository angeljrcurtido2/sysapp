...
const ListarVentas = () => {
  ...
  const handleDownloadPDF = async (tipoFiltro: string) => {
    try {
      ...
      console.log('Respuesta del servidor para PDF:', res.data);
      ...
      const filename = FileSystem.documentDirectory + `Reporte-Ventas-${tipoFiltro}-${fecha}.pdf`;
      ...
      if (Platform.OS === "ios") {
        ...
        if (permissions.granted) {
          await FileSystem.StorageAccessFramework.createFileAsync(
            ...
            `Reporte-Ventas-${tipoFiltro}-${fecha}.pdf`,
            'application/pdf'
          )
            .then(async (uri) => {
